...
namespace Queue
{
    class OperatorSessionService
    {
        ...
        public OperatorSessionService(OperatorSessionManager sessionManager, TicketStateManager ticketManager, ConnectionPool connectionPool)
        {
        ...
        public ZMessage Process(ZMessage zmsg)
        {
            ...
            var msg = JsonConvert.DeserializeObject<OperatorSessionEventMsg>(body);

            var reply = client.SendToWorker(new ZMessage(msg.SessionId));
            if (reply.BodyToString() != ReplyStatus.OK)
            {
                return zmsg;
            }
            try
            ...
            {
                client.SendToWorker(new ZMessage(msg.SessionId));
            }
            return null;
        }
    }
